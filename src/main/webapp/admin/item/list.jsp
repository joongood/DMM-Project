...
<!-- 필요한 스크립트를 포함 -->
    ...
    //상품 정보 상세 보기
    function popup(str){
    	window.open("view?itemId="+str,"view","width=600,height=650,left="+popupX+",top="+popupY);
    }
    ...
    function id_delete(str){
    	...
    	if(result == true){
    		location.href="delete?itemId="+str;
    	}    	
    }    
</script>
    <div class="container">
        ...
        <div class="content">
            <h2>상품 목록</h2>            
            <div>            
            	<div>총 상품 수 : ${item_count }</div>
            	<div>
            		<form>
            		<div align=right>
            			<span>
            				<select name="field" style="height:20px;">
								<option value="seller_id" <c:if test="${field == 'sellerId' }">selected</c:if>>판매자</option>
								<option value="ca_id" <c:if test="${field == 'caId' }">selected</c:if>>분류코드</option>
								<option value="location" <c:if test="${field == 'location' }">selected</c:if>>판매지역</option>
							</select>
        ...
        <table style="width:100%; height:15px; color:white; font-size:12px;">
        	<tr bgcolor="#40BEA7" height="30">
        		...
        		<td style="color:white">분류코드</td>
        		<td style="color:white">가격</td>
        		<td style="color:white">상품 상태</td>
        		<td style="color:white">올라온 날짜</td>
        		<td style="color:white">판매 상태</td>
        		<td style="color:white">판매 장소</td>
        ...
        <c:set var="number" value="${number }" />
			...
			<tr height="24">
				...
				<td align="center">${list.caId }</td>
				<td align="center">${list.price }</td>
				<td align="center">
				<c:choose>
						<c:when test="${list.conditionItem eq '판매중'}">
							<font color=green>
								${list.conditionItem }
							</font>
						</c:when>
						<c:when test="${list.conditionItem eq '판매완료'}">
							<font color=blue>					
								${list.conditionItem }
							</font>
						</c:when>
				...
				<td align="center">
					<span onclick="popup('${list.itemId }')" style="cursor:pointer">[상세]</span>
					<span onclick="id_delete('${list.itemId }')" style="cursor:pointer">[삭제]</span>
				</td>
			</tr>
			<tr><td colspan=9 style="bgcolor:#e5ecef;"></td></tr>
			...
			<tr>
				<td>
					<c:if test="${item_count>0 }">
						<c:set var="pageCount" value="${item_count / pageSize + (item_count % pageSize == 0 ? 0 : 1) }" />
